...
function showProducts(searchTerm = '') {
    ...
    let productsToShow = [];
    const isAllProductsPage = window.location.pathname.includes('allProducts.html');
    
    if (isAllProductsPage) {
        // Show all products on allProducts.html
        productsToShow = filteredProducts;
    } else {
        // Show 4 fixed + 4 random on products.html (max 8 total)
        const fixedProducts = filteredProducts.slice(0, 4);
        const remainingProducts = filteredProducts.slice(4);
        const randomProducts = remainingProducts
            .sort(() => 0.5 - Math.random())
            .slice(0, Math.min(4, remainingProducts.length));
        productsToShow = [...fixedProducts, ...randomProducts];
    }
